...
export default defineBackground(() => {
  ...
  function addRules(rules: ApiRuleItem[]) {
      ...
      addRules: rules.map((rule) => {
        return {
          ...
          action: {
            type: "redirect",
            redirect: {
              url: `data:application/json;charset=utf-8;base64,${stringToBase64(JSON.stringify(rule.mock))}`,
            },
          },
          condition: {
            urlFilter: rule.path,
  ...
  function removeRules(ids: number[]) {
    return browser.declarativeNetRequest.updateDynamicRules({
      removeRuleIds: ids,
    });
  }
